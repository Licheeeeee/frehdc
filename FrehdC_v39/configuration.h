...
typedef struct Config
{
  ...
  char saveFolder[100], inputFolder[100], restartFile[60];
  // use the cell edge file
  int useCellEdge, isRestart, ttRestart;
  ...
  int inflowN, inflowLoc[1000], windspdN, winddirN, evapN, rainN;
  ...
  int tidalPSN, tidalMSN, scalarAdv;
  ...
  int staggeredV;
  // use low cutoff for subgrid A/V ratio or not, and the cutoff value
  int useAVcutoff;
  double effHmin;
  // -------------------- MPI settings -----------------------------------------
  int useMPI, np;
