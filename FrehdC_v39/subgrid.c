...
void updateVelocitySub(Data **data, Maps *map, Sub *sub, Config *setting)
{
  ...
  if (setting->useSubgrid == 1)
  {
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      // if (ii == 189*88+40-1)
      // {printf("En, Baro = %f, %f\n",(*data)->EnYP[ii], - (setting->g * setting->dt * effHy) \
      //   * ((*data)->surf[map->icjP[ii]] - (*data)->surf[ii]));}

      // do not use subgrid method at wetting/drying front
      /*if (sub->Vx[ii] > 0)
      {
        if (((*data)->depthXP[ii] > 0) & (sub->V[ii] == 0 | sub->V[map->iPjc[ii]] == 0))
        {
...
void updateCDSub(Data **data, Sub *sub, Config *setting)
{
  ...
  if (setting->CDnotN == 0 | setting->CDnotN == 1)
  {
    ...
    else if (setting->useSubDrag == 3)
    {
      ...
      for (ii = 0; ii < setting->N2ci; ii++)
      {
        if (sub->V[ii] > 0)
        {
          ...
          // Combine Volp2013 with Li2018, ZhiLi20180414
          // if (sub->Yh[ii] > 0)
          // {
          //   im = ii - setting->nx;
          //   ip = ii + setting->nx;
          //   (*data)->CDXP[ii] = coeff*sub->Yh[ii] + \
          //     setting->lambda1 * (sub->CdsX[ii] + 0.5*sub->CdsX[im] + 0.5*sub->CdsX[ip]);
          //   if ((*data)->CDXP[ii] > setting->CDmax) {(*data)->CDXP[ii] = setting->CDmax;}
          //   im = ii - 1;
          //   ip = ii + 1;
          //   (*data)->CDYP[ii] = coeff*sub->Yh[ii] + \
          //     setting->lambda1 * (sub->CdsY[ii] + 0.5*sub->CdsY[im] + 0.5*sub->CdsY[ip]);
          //   if ((*data)->CDYP[ii] > setting->CDmax) {(*data)->CDYP[ii] = setting->CDmax;}
          // }
          // else
          // {
          //   (*data)->CDXP[ii] = 0;
