...
void computeSubArea(Sub **sub, Data *data, Bath *bath, Maps *map, Config *setting)
{
  ...
  */
  ...
  for (ii = 0; ii < setting->N2ci; ii++)
  {
    ...
    {
      (*sub)->YhX[ii] = (*sub)->allYhX[k1];
      (*sub)->YhY[ii] = (*sub)->allYhY[k1];
    }
 ...
 (*sub)->minVxp[ii] = interpSurf(data->surf[ii],(*sub)->minVxp[ii],(*sub)->allSurf[j1], \
      (*sub)->allminVxp[k1],(*sub)->allSurf[j2],(*sub)->allminVxp[k2]);
 (*sub)->minVyp[ii] = interpSurf(data->surf[ii],(*sub)->minVyp[ii],(*sub)->allSurf[j1], \
      (*sub)->allminVyp[k1],(*sub)->allSurf[j2],(*sub)->allminVyp[k2]);
    (*sub)->minVxm[ii] = interpSurf(data->surf[ii],(*sub)->minVxm[ii],(*sub)->allSurf[j1], \
      (*sub)->allminVxm[k1],(*sub)->allSurf[j2],(*sub)->allminVxm[k2]);
    (*sub)->minVym[ii] = interpSurf(data->surf[ii],(*sub)->minVym[ii],(*sub)->allSurf[j1], \
      (*sub)->allminVym[k1],(*sub)->allSurf[j2],(*sub)->allminVym[k2]);
    ...
    if (setting->useminA != 0 | setting->useminV != 0)
      {
	...
	(*sub)->Nmax[ii] = interpSurf(data->surf[ii],(*sub)->Nmax[ii],(*sub)->allSurf[j1], \
                                    (*sub)->allNmax[k1],(*sub)->allSurf[j2],(*sub)->allNmax[k2]);
	(*sub)->Omax[ii] = interpSurf(data->surf[ii],(*sub)->Omax[ii],(*sub)->allSurf[j1], \
                                  (*sub)->allOmax[k1],(*sub)->allSurf[j2],(*sub)->allOmax[k2]);
      }  
...
{
  int ii, j1, j2, k1, k2, v1, v2, kk, eqind, flag = 0;
  int useEffVx, useEffVy;
  //double *eqsurf = malloc(sizeof(double));
  double eqN, realN, eqV, lambda, beta = 0.0;
  double Vmin = 0.002;
  double Amin = 0.002;
  double Azmin = 0.002;
  double epsi = 0.001;
  ...
  for (ii = 0; ii < setting->N2ci; ii++)
  {
    ...
    if (map->iPjc[ii] < setting->N2ci)
    {
      ...
      // Vmin
      if (setting->useminV != 0)
	{(*sub)->minVx[ii] = (*sub)->minVxp[ii] + (*sub)->minVxm[map->iPjc[ii]];}
    ...
    if (map->icjP[ii] < setting->N2ci)
    {
      ...
      // Vmin
      if (setting->useminV != 0)
	{(*sub)->minVy[ii] = (*sub)->minVyp[ii] + (*sub)->minVym[map->icjP[ii]];}
      // if waterfall condition is met, only use half of the volumes and areas
    ...
    else
    {
      ...
      (*sub)->minVy[ii] = 2 * (*sub)->minVyp[ii];
    }

    //(*sub)->Vx[ii] = (*sub)->Vx[ii] * 0.855;
    //(*sub)->Vy[ii] = (*sub)->Vy[ii] * 0.855;
   
      // ZhiLi20181004
      // adjust the face area with Omin and Nmin, added by ZhiLi20181004
      if (setting->useminA == 1 & beta >= 0)
      {
	//printf("1\n");
	useEffVx = 0;
	useEffVy = 0;
	if (((*sub)->Nx[ii] - (*sub)->Nmax[ii] > epsi) & ((*sub)->Nmax[ii] > 0))
          {(*sub)->Nx[ii] = (*sub)->Nx[ii] - setting->beta * ((*sub)->Nx[ii] - (*sub)->Nmax[ii]); useEffVx = 1;}
	if (((*sub)->Oy[ii] - (*sub)->Omax[ii] > epsi) & ((*sub)->Omax[ii] > 0))
	  {(*sub)->Oy[ii] = (*sub)->Oy[ii] - setting->beta * ((*sub)->Oy[ii] - (*sub)->Omax[ii]); useEffVy = 1;}
      }
      else if (setting->useminA == 1 & beta < 0)
	{
	  if (((*sub)->Nx[ii] - (*sub)->Nmin[ii] > epsi) & ((*sub)->Nmin[ii] > 0) & ((*sub)->Nx[ii] > 0))
	    {(*sub)->Nx[ii] = 2.0 * (*sub)->Nmin[ii] - (*sub)->Nmin[ii]*(*sub)->Nmin[ii]/(*sub)->Nx[ii];}
	  if (((*sub)->Oy[ii] - (*sub)->Omin[ii] > epsi) & ((*sub)->Omin[ii] > 0) & ((*sub)->Oy[ii] > 0))
	    {(*sub)->Nx[ii] = 2.0 * (*sub)->Omin[ii] - (*sub)->Omin[ii]*(*sub)->Omin[ii]/(*sub)->Oy[ii];}
	}

      // adjust the cell volume, ZhiLi20190125
      if (setting->useminV == 1)
	{
	  // use effective volume only at channel contraction
	  lambda = 1.0;
	  if (useEffVx == 1)
	    {(*sub)->Vx[ii] = (1-lambda)*(*sub)->Vx[ii] + lambda*(*sub)->minVx[ii]; useEffVx = 0; }
	  if (useEffVy == 1)
	    {(*sub)->Vy[ii] = (1-lambda)*(*sub)->Vy[ii] + lambda*(*sub)->minVy[ii]; useEffVy = 0; }
	}
      else if (setting->useminV == 2)
	{
	  // use effective volume for all grid cells
	  lambda = 1.0;
	  
	  //if ((*sub)->Nmax[ii] > 0 & (*sub)->Nx[ii] - (*sub)->Nmax[ii] > epsi)
	  {(*sub)->Vx[ii] = (1 - lambda)*(*sub)->Vx[ii] + lambda*(*sub)->minVx[ii];}
	    //if ((*sub)->Omax[ii] > 0 & (*sub)->Oy[ii] - (*sub)->Omax[ii] > epsi)
	    {(*sub)->Vy[ii] = (1 - lambda)*(*sub)->Vy[ii] + lambda*(*sub)->minVy[ii];}
	   
      	}
      else if (setting->useminV == 3)
	{
	   lambda = 1.0;
	  if (useEffVx == 1)
	    {
	      (*sub)->Vx[ii] = (1-lambda)*(*sub)->Vx[ii] + lambda*(*sub)->minVx[ii];
	      (*sub)->Zx[ii] = (*sub)->Zx[ii] * (*sub)->Nmax[ii] / (*sub)->Nx[ii];
	      useEffVx = 0; 
	    }
	  if (useEffVy == 1)
	    {
	      (*sub)->Vy[ii] = (1-lambda)*(*sub)->Vy[ii] + lambda*(*sub)->minVy[ii];
	      (*sub)->Zy[ii] = (*sub)->Zy[ii] * (*sub)->Omax[ii] / (*sub)->Oy[ii];
	      useEffVy = 0;
	    }
	}
  }
  ...
  for (ii = 0; ii < setting->ny; ii++)
  {
    ...
    (*sub)->minVx[map->iMgt[ii]] = (*sub)->minVy[map->iMbd[ii]];
    (*sub)->minVy[map->iMgt[ii]] = (*sub)->minVx[map->iMbd[ii]];
    ...
    (*sub)->minVy[map->iPgt[ii]] = (*sub)->minVx[map->iPbd[ii]];
    (*sub)->minVx[map->iPgt[ii]] = (*sub)->minVy[map->iPbd[ii]];
    (*sub)->Z[map->iPgt[ii]] = (*sub)->Z[map->iPbd[ii]];
    if (setting->useSubDrag == 2)
    {
      (*sub)->YhX[map->iPgt[ii]] = (*sub)->YhX[map->iPbd[ii]];
      (*sub)->YhY[map->iPgt[ii]] = (*sub)->YhY[map->iPbd[ii]];
    }
    if (setting->useSubDrag == 3)
    {
  ...
  for (ii = 0; ii < setting->nx; ii++)
  {
    ...
    (*sub)->minVx[map->jMgt[ii]] = (*sub)->minVx[map->jMbd[ii]];
    (*sub)->minVy[map->jMgt[ii]] = (*sub)->minVy[map->jMbd[ii]];
    ...
    (*sub)->minVx[map->jPgt[ii]] = (*sub)->minVx[map->jPbd[ii]];
    (*sub)->minVy[map->jPgt[ii]] = (*sub)->minVy[map->jPbd[ii]];
    (*sub)->Z[map->jPgt[ii]] = (*sub)->Z[map->jPbd[ii]];
    if (setting->useSubDrag == 2)
    {
      (*sub)->YhX[map->jPgt[ii]] = (*sub)->YhX[map->jPbd[ii]];
      (*sub)->YhY[map->jPgt[ii]] = (*sub)->YhY[map->jPbd[ii]];
    }
    if (setting->useSubDrag == 3)
    {
...
{
    int ii;
    double advX, advY, alpha = 0.707;
    double CFLh = 0.7, CFLl = 0.5, CFLx, CFLy, vDiag;
    for (ii = 0; ii < setting->N2ci; ii++)
      {
	...
	if ((*data)->uuXP[ii] > 0)
	  {
	    // if the upwind face is not blocked, this is the traditional upwind scheme
	    if (sub->Nx[map->iMjc[ii]] > 0)
	      { (*data)->advX[ii] = (1.0/setting->dx)*((*data)->uuXP[ii])*((*data)->uuXP[ii]-(*data)->uuXP[map->iMjc[ii]]); }
	    // if all the rest faces area blocked, no advection occurs
	    else if (sub->Oy[map->icjM[ii]] <= 0 & sub->Oy[ii] <= 0)
	      { (*data)->advX[ii] = 0; }
	    // if the upwind face is blocked, but at least one of the side faces > 0
	    else
	      {
		// if only 1 of the side faces > 0, advection still exists
		if (sub->Oy[ii] <= 0)
		  {
			  vDiag = 0.5 * ((*data)->vvYP[map->icjM[ii]] + (*data)->uuXP[ii]);
			  (*data)->advX[ii] = (2.0/setting->dx)*((*data)->uuXP[ii])*((*data)->uuXP[ii] - alpha*vDiag);
		  }
		else if (sub->Oy[map->icjM[ii]] <= 0)
		  {
			  vDiag = 0.5 * (-(*data)->vvYP[ii] + (*data)->uuXP[ii]);
			  (*data)->advX[ii] = (2.0/setting->dx)*((*data)->uuXP[ii])*((*data)->uuXP[ii] + alpha*vDiag);
		  }
		// otherwise(both Oy faces are > 0), no advection occurs in x direction
		else
		  {(*data)->advX[ii] = 0;}
	      }
	  }
	else if ((*data)->uuXP[ii] < 0)
	  {
	    if (sub->Nx[map->iPjc[ii]] > 0)
	      { (*data)->advX[ii] = (1.0/setting->dx)*((*data)->uuXP[ii])*((*data)->uuXP[map->iPjc[ii]]-(*data)->uuXP[ii]);}
	    else if (ii+1 < setting->N2ci & map->icjM[ii]+1 < setting->N2ci & sub->Oy[map->icjM[ii]+1] <= 0 & sub->Oy[ii+1] <= 0)
	      { (*data)->advX[ii] = 0;}
	    else
	      {
		if (ii+1 < setting->N2ci & sub->Oy[ii+1] <= 0)
		  {
			  vDiag = 0.5 * (-(*data)->vvYP[map->icjM[ii]+1] + (*data)->uuXP[ii]);
			  (*data)->advX[ii] = (2.0/setting->dx)*((*data)->uuXP[ii])*(- alpha*vDiag - (*data)->uuXP[ii]);
		  }
		else if (map->icjM[ii]+1 < setting->N2ci & sub->Oy[map->icjM[ii]+1] <= 0)
		  {
			  vDiag = 0.5 * ((*data)->vvYP[ii+1] + (*data)->uuXP[ii]);
			  (*data)->advX[ii] = (2.0/setting->dx)*((*data)->uuXP[ii])*(alpha*vDiag - (*data)->uuXP[ii]);
		  }
		else
		  {(*data)->advX[ii] = 0;}
	...
	if ((*data)->vvXP[ii] > 0)
	  {
	    // if face iMjc is not blocked, can compute advection
	    if (sub->Nx[map->icjM[ii]] > 0)
	      { (*data)->advX[ii] = (*data)->advX[ii] + (1.0/setting->dy)*((*data)->vvXP[ii])*((*data)->uuXP[ii]-(*data)->uuXP[map->icjM[ii]]);}
	    // otherwise, no advection occurs
	  }
	else if ((*data)->vvXP[ii] < 0)
	  {
	    if (sub->Nx[map->icjP[ii]] > 0)
	      { (*data)->advX[ii] = (*data)->advX[ii] + (1.0/setting->dy)*((*data)->vvXP[ii])*((*data)->uuXP[map->icjP[ii]]-(*data)->uuXP[ii]);}
	      }
	...
	if ((*data)->vvYP[ii] > 0)
	  {
	    // if the upwind face is not blocked, this is the traditional upwind scheme
	    if (sub->Oy[map->icjM[ii]] > 0)
	      { (*data)->advY[ii] = (1.0/setting->dy)*((*data)->vvYP[ii])*((*data)->vvYP[ii]-(*data)->vvYP[map->icjM[ii]]);}
	    // if all the rest faces area blocked, no advection occurs
	    else if (sub->Nx[map->iMjc[ii]] <= 0 & sub->Nx[ii] <= 0)
	      { (*data)->advY[ii] = 0;}
	    // if the upwind face is blocked, but at least one of the side faces > 0
	    else
	      {
		...
		if (sub->Nx[ii] <= 0)
		  {
			  vDiag = 0.5 * ((*data)->uuXP[map->iMjc[ii]] + (*data)->vvYP[ii]);
			  (*data)->advY[ii] = (2.0/setting->dy)*((*data)->vvYP[ii])*((*data)->vvYP[ii] - alpha*vDiag);
		  }
		else if (sub->Nx[map->iMjc[ii]] <= 0)
		  {
			  vDiag = 0.5 * (-(*data)->uuXP[ii] + (*data)->vvYP[ii]);
			  (*data)->advY[ii] = (2.0/setting->dy)*((*data)->vvYP[ii])*((*data)->vvYP[ii] + alpha*vDiag);
		  }
		// otherwise, no advection occurs
		else
	...
	else if ((*data)->vvYP[ii] < 0)
	  {
	    if (sub->Oy[map->icjP[ii]] > 0)
	      { (*data)->advY[ii] = (1.0/setting->dy)*((*data)->vvYP[ii])*((*data)->vvYP[map->icjP[ii]]-(*data)->vvYP[ii]);}
	    else if (sub->Nx[map->iMjc[ii]+setting->NX] <= 0 & sub->Nx[ii+setting->NX] <= 0)
	      { (*data)->advY[ii] = 0;}
	    else
	      {
		if (sub->Nx[ii+setting->NX] <= 0)
		  {
			  vDiag = 0.5 * (-(*data)->uuXP[map->iMjc[ii]+setting->NX] + (*data)->vvYP[ii]);
			  (*data)->advY[ii] = (2.0/setting->dy)*((*data)->vvYP[ii])*(-alpha*vDiag - (*data)->vvYP[ii]);
		  }
		else if (sub->Nx[map->iMjc[ii]+setting->NX] <= 0)
		  {
			  vDiag = 0.5 * ((*data)->uuXP[ii+setting->NX] + (*data)->vvYP[ii]);
			  (*data)->advY[ii] = (2.0/setting->dy)*((*data)->vvYP[ii])*(alpha*vDiag - (*data)->vvYP[ii]);
		  }
		else
		  {(*data)->advY[ii] = 0;}
	...
	if ((*data)->uuYP[ii] > 0)
	  {
	    // if face icjM is not blocked, can compute advection
	    if (sub->Oy[map->iMjc[ii]] > 0)
	      { (*data)->advY[ii] = (*data)->advY[ii] + (1.0/setting->dx)*((*data)->uuYP[ii])*((*data)->vvYP[ii]-(*data)->vvYP[map->iMjc[ii]]);}
	    // otherwise, no advection occurs
	  }
	else if ((*data)->uuYP[ii] < 0)
	  {
	    if (sub->Oy[map->iPjc[ii]] > 0)
	      { (*data)->advY[ii] = (*data)->advY[ii] + (1.0/setting->dx)*((*data)->uuYP[ii])*((*data)->vvYP[map->iPjc[ii]]-(*data)->vvYP[ii]);}
	      }
      }
    /*for (ii = 0; ii < setting->N2ci; ii++)
    {
...
void matrixCoeffSub(Data **data, Maps *map, Sub *sub, Config *setting)
{
  ...
  if (setting->useSubgrid == 1)
  {
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      {
        if (sub->Vx[ii] > 0)
        {(*data)->GnXP[ii] = coefx * sub->Nx[ii] * sub->Nx[ii] / sub->Vx[ii] / divider / (1 + (*data)->DragXP[ii]);}
        if (sub->Vx[jj] > 0)
        {(*data)->GnXM[ii] = coefx * sub->Nx[jj] * sub->Nx[jj] / sub->Vx[jj] / divider / (1 + (*data)->DragXP[jj]);}
        if (sub->Vy[ii] > 0)
        {(*data)->GnYP[ii] = coefy * sub->Oy[ii] * sub->Oy[ii] / sub->Vy[ii] / divider / (1 + (*data)->DragYP[ii]);}
        if (sub->Vy[kk] > 0)
        {(*data)->GnYM[ii] = coefy * sub->Oy[kk] * sub->Oy[kk] / sub->Vy[kk] / divider /  (1 + (*data)->DragYP[kk]);}
        (*data)->GnCt[ii] = sub->Z[ii] + (*data)->GnXP[ii] + (*data)->GnYP[ii] + (*data)->GnXM[ii] + \
          (*data)->GnYM[ii];
      }
      else
  ...
  else if (setting->useSubgrid == 2)
  {
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      {(*data)->GnXM[ii] = coefx * sub->Nx[jj] / setting->dx / \
        (1 + (*data)->DragXP[map->iMjc[ii]]);}
      if (sub->Vy[ii] > 0)
      {(*data)->GnYP[ii] = coefy * sub->Oy[ii] / setting->dy / (1 + (*data)->DragYP[ii]);}
      if (sub->Vy[kk] > 0)
      {(*data)->GnYM[ii] = coefy * sub->Oy[jj] / setting->dy / \
        (1 + (*data)->DragYP[map->icjM[ii]]);}
  ...
  for (ii = 0; ii < setting->N2ci; ii++)
  {
    ...
    //       printf("ii,Xm,Ym,Ct,Xp,Yp,z=%d,%f,%f,%f,%f,%f,%f\n",ii,(*data)->GnXM[ii],(*data)->GnYM[ii],(*data)->GnCt[ii],(*data)->GnXP[ii],(*data)->GnYP[ii],(*data)->z[map->sprt[ii]]);
  }
}
...
//          {effHy = setting->effHmin;}
      ...
      (*data)->uuXP[ii] = (*data)->EnXP[ii] - (setting->g * setting->dt * effHx) \
        * ((*data)->surf[map->iPjc[ii]] - (*data)->surf[ii]) / divider;// / (1.0 + (*data)->DragXP[ii]) ;
      (*data)->vvYP[ii] = (*data)->EnYP[ii] - (setting->g * setting->dt * effHy) \
        * ((*data)->surf[map->icjP[ii]] - (*data)->surf[ii]) / divider;// / (1.0 + (*data)->DragYP[ii]) ;
    }
  }
...
void initAllSubVar(Sub **sub, Bath *bath, Config *setting, int irank)
{
    ...
    *sub = malloc(sizeof(Sub));
	...
	initOneSubVar(sub, "Nmax.dat", setting, irank);
	(*sub)->allNmax = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
	for (ii = 0; ii < N; ii++)
	  {(*sub)->allNmax[ii] = (*sub)->allvar[ii];}
	initOneSubVar(sub, "Omax.dat", setting, irank);
	(*sub)->allOmax = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
	for (ii = 0; ii < N; ii++)
	  {(*sub)->allOmax[ii] = (*sub)->allvar[ii];}
      }
    // min volume, added by ZhiLi20190125
    initOneSubVar(sub, "minVxp.dat", setting, irank);
    (*sub)->allminVxp = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
    for (ii = 0; ii < N; ii++)
    {(*sub)->allminVxp[ii] = (*sub)->allvar[ii];}
    initOneSubVar(sub, "minVxm.dat", setting, irank);
    (*sub)->allminVxm = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
    for (ii = 0; ii < N; ii++)
    {(*sub)->allminVxm[ii] = (*sub)->allvar[ii];}
    initOneSubVar(sub, "minVyp.dat", setting, irank);
    (*sub)->allminVyp = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
    for (ii = 0; ii < N; ii++)
    {(*sub)->allminVyp[ii] = (*sub)->allvar[ii];}
    initOneSubVar(sub, "minVym.dat", setting, irank);
    (*sub)->allminVym = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
    for (ii = 0; ii < N; ii++)
    {(*sub)->allminVym[ii] = (*sub)->allvar[ii];}

    // drag model by Volp
    if (setting->useSubDrag == 2)
    {
        initOneSubVar(sub, "YhX.dat", setting, irank);
        (*sub)->allYhX = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
        for (ii = 0; ii < N; ii++)
        {(*sub)->allYhX[ii] = (*sub)->allvar[ii];}
	initOneSubVar(sub, "YhY.dat", setting, irank);
	(*sub)->allYhY = malloc(setting->N2ci*(*sub)->Ne*sizeof(double));
	for (ii = 0; ii < N; ii++)
	  {(*sub)->allYhY[ii] = (*sub)->allvar[ii];}
    }
    // drag model by Casas
    if (setting->useSubDrag == 4 | setting->useSubDrag == 3)
    {
    ...
    // drag model based on roughness height, ZhiLi20181028
    if (setting->useSubDrag == 5)
      {
	char fpath[100], fname[100];
	strcpy(fpath, setting->inputFolder);
	strcat(fpath, setting->subgridFolder);
	strcpy(fname, fpath);
	strcat(fname, "Z0.dat");
	...
	ReadFile((*sub)->allZ0, fname, setting->N2CI);
    ...
    (*sub)->Nmax = malloc(setting->N2ct*sizeof(double));
    (*sub)->Omax = malloc(setting->N2ct*sizeof(double));
    // min volume, ZhiLi20190125
     (*sub)->minVyp = malloc(setting->N2ct*sizeof(double));
    (*sub)->minVxp = malloc(setting->N2ct*sizeof(double));
    (*sub)->minVym = malloc(setting->N2ct*sizeof(double));
    (*sub)->minVxm = malloc(setting->N2ct*sizeof(double));
    (*sub)->minVy = malloc(setting->N2ct*sizeof(double));
    (*sub)->minVx = malloc(setting->N2ct*sizeof(double));

    if (setting->useSubDrag == 2)
    {
      (*sub)->YhX = malloc(setting->N2ct*sizeof(double));
      (*sub)->YhY = malloc(setting->N2ct*sizeof(double));
    }
    if (setting->useSubDrag == 4 | setting->useSubDrag == 3)
    {(*sub)->Cn = malloc(setting->N2ct*sizeof(double));}
    if (setting->useSubDrag == 3)
