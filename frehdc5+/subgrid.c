...
void combineSubArea(Sub **sub, Data *data, Bath *bath, Maps *map, Config *setting, int irank, int nrank)
{
  ...
  for (ii = 0; ii < setting->N2ci; ii++)
  {
    ...
    else
    {
      ...
      if (setting->useminA == 1 & beta >= 0)
      {
		if (((*sub)->Nx[ii] - (*sub)->Nmax[ii] > epsi) & ((*sub)->Nmax[ii] > 0))
		      {(*sub)->Nx[ii] = (*sub)->Nx[ii] - setting->beta * ((*sub)->Nx[ii] - (*sub)->Nmax[ii]);}
		if (((*sub)->Oy[ii] - (*sub)->Omax[ii] > epsi) & ((*sub)->Omax[ii] > 0))
		  {(*sub)->Oy[ii] = (*sub)->Oy[ii] - setting->beta * ((*sub)->Oy[ii] - (*sub)->Omax[ii]);}
	  }
      ...
      if (setting->useminV == 1)
	{
	  double Vmin, r;
	  Vmin = 1.0;
	  if (((*sub)->Nmin[ii] > 0) & ((*sub)->Vx[ii] > (*sub)->minVx[ii]) & ((*sub)->minVx[ii] > 0))
	    {    
	      r = (*sub)->Nmin[ii] / (*sub)->Nmax[ii];
	      if (r>0 & r<1) 
		{ 
		  lambda = -5.01*pow(1-r,2) + 6.35*(1-r) - 0.6;
		  if (1-r > 0.67) {lambda = 1.35;}
		  else if (lambda < 0) {lambda = 0.0;}
		}
	      else {lambda = 0.0;}
	      (*sub)->Vx[ii] = (*sub)->Vx[ii] - (1-r) * lambda * (*sub)->Vx[ii];
	      if ((*sub)->Vx[ii] < Vmin) {(*sub)->Vx[ii] = Vmin;}
	    }
	  if (((*sub)->Omin[ii] > 0) & ((*sub)->Vy[ii] > (*sub)->minVy[ii]) & ((*sub)->minVy[ii] > 0))
	    {
	      r = (*sub)->Omin[ii] / (*sub)->Omax[ii];
	      if (r>0 & r<1) 
		{ 
		  lambda = -5.01*pow(1-r,2) + 6.35*(1-r) - 0.6;
		  if (1-r > 0.67) {lambda = 1.4;}
		  else if (lambda < 0) {lambda = 0.0;}
		}
	      else {lambda = 0.0;}
	      (*sub)->Vy[ii] = (*sub)->Vy[ii] - (1-r) * lambda * (*sub)->Vy[ii];
	      if ((*sub)->Vy[ii] < Vmin) { (*sub)->Vy[ii] = Vmin; }
	    }
	}
      else if (setting->useminV == 2)
	{
	  //if ((*sub)->Nmax[ii] > 0 & (*sub)->Nx[ii] - (*sub)->Nmax[ii] > epsi)
	  {(*sub)->Vx[ii] = (*sub)->minVx[ii];}
	    //if ((*sub)->Omax[ii] > 0 & (*sub)->Oy[ii] - (*sub)->Omax[ii] > epsi)
	    {(*sub)->Vy[ii] = (*sub)->minVy[ii];}
	}
...
void updateCDSub(Data **data, Sub *sub, Maps *map, Config *setting)
{
  ...
  if (setting->CDnotN == 0 | setting->CDnotN == 2)
  {
    ...
    else if (setting->useSubDrag == 4 | setting->useSubDrag == 3)
    {
        for (ii = 0; ii < setting->N2ci; ii++)
        {
            if (sub->V[ii] > 0)
