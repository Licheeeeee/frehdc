...
void zeroNegSurf(Data **data, Bath *bath, Config *setting);
...
void evaporation(Data **data, BC *bc, int tt, Config *setting);
...
void monitorCFL(Data **data, int irank, Config *setting, int tt);
...
void windTerm(Data **data, BC *bc, Config *setting, int tt)
{
  double phi, omega, tau, Cw, pi = 3.1415926, rho = 1000;
  int ii;
  if (setting->useWind == 1)
  {
    ...
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      if ((*data)->depth[ii] > 0)
      {
        (*data)->EnXP[ii] = (*data)->EnXP[ii] + setting->dt * tau * cos(omega) / \
          ((*data)->depth[ii] * rho);
        (*data)->EnYP[ii] = (*data)->EnYP[ii] + setting->dt * tau * sin(omega) / \
          ((*data)->depth[ii] * rho);
      }
    }
  }
...
// =============== Compute the evaporation ===============
void evaporation(Data **data, BC *bc, int tt, Config *setting)
{
  int ii;
  double h = bc->evap[tt];
  for (ii = 0; ii < setting->N2ci; ii++)
  {(*data)->surf[ii] -= h*setting->dt;}
}
...
{
  int ii;
  double Cw = 0.7, Ck = 0.1, Cv = 0, CFLmax = 0.24, surf, bot, dep;
...
void monitorCFL(Data **data, int irank, Config *setting, int tt)
{
  ...
  if (fabs(CFLx0) > 1000 | fabs(CFLy0) > 1000)
  {
    printf("WARNING: Max CFL exceeds 1000, program forced to quit!\n\n");
    exit(EXIT_FAILURE);
  }
}
