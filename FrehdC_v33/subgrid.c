...
void updateCDSub(Data **data, Sub *sub, Config *setting);
...
void computeSubArea(Sub **sub, Data *data, Bath *bath, Maps *map, Config *setting)
{
  ...
  */
  ...
  for (ii = 0; ii < setting->N2ci; ii++)
  {
    ...
    // bottom curvature, since model is not very sensitive to bottom curvature,
    // linear interpolation is not used here
...
void combineSubArea(Sub **sub, Data *data, Bath *bath, Maps *map, Config *setting, int irank, int nrank)
{
  ...
  double eqN, realN, eqV, alpha = setting->subAlpha;
  double Vmin = 0.002;
  double Amin = 0.002;
  double Azmin = 0.002;
...
void advectionTermSub(Data **data, Maps *map, Sub *sub, Config *setting)
{
  ...
  for (ii = 0; ii < setting->N2ci; ii++)
  {
    ...
    if ((*data)->vvYP[ii] == 0) {advY = 0;}
      if (ii == 18612) {printf("uuXP, vvXP, advX, Vx = %lf,%lf,%lf,%lf\n",(*data)->uuXP[ii],(*data)->vvXP[ii],advX,sub->Vx[ii]);}
...
void windTermSub(Data **data, BC *bc, Sub *sub, Config *setting, int tt)
{
  double phi, omega, tau, Cw, pi = 3.1415926, rho = 1000.0;
  int ii;
  if (setting->useWind == 1)
  {
    ...
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      if ((*data)->depth[ii] < setting->hD & setting->useThinLayer == 1)
      {tau = tau * exp(setting->CwT*((*data)->depth[ii]-setting->hD)/setting->hD);}
      // add wind drag to the source term
      if (sub->Vx[ii] > 0)
      {
        (*data)->EnXP[ii] = (*data)->EnXP[ii] + setting->dt * tau * cos(omega) * sub->Zx[ii] / \
          (sub->Vx[ii] * rho);
      }
      if (sub->Vy[ii] > 0)
      {
        (*data)->EnYP[ii] = (*data)->EnYP[ii] + setting->dt * tau * sin(omega) * sub->Zy[ii] / \
          (sub->Vy[ii] * rho);
      }
    }
  }
...
void matrixSourceTermSub(Data **data, Maps *map, BC *bc, Sub *sub, Config *setting, int tt, int irank)
{
  ...
  for (ii = 0; ii < setting->N2ci; ii++)
  {if ((*data)->z[ii] == 0) {(*data)->z[ii] = (*data)->surf[map->trps[ii]];}}
}
...
{
  int ii,jj,kk;
  double coefx, coefy, Gmin = 0.00000001;
  ...
  if (setting->useSubgrid == 1)
  {
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      if ((*data)->GnCt[ii] == 0.0)
      {(*data)->GnCt[ii] = 1.0;}// (*data)->z[ii] = (*data)->surf[map->trps[ii]];}
      ...
      if ((*data)->depth[ii] == 0)
      {
            ...
            (*data)->vvYP[map->icjM[ii]] == 0)
        {
            ...
            (*data)->GnYM[ii] = 0.0;
            (*data)->GnYP[ii] = 0.0;
            (*data)->z[map->sprt[ii]] = (*data)->surf[ii];
        }
      }
    }
  }
  else if (setting->useSubgrid == 2)
  {
    ...
    (*data)->GnYP[ii] = 0;
    (*data)->GnYM[ii] = 0;
    (*data)->GnXP[ii] = coefx * sub->Nx[ii] / setting->dx;
    (*data)->GnXM[ii] = coefx * sub->Nx[jj] / setting->dx;
    (*data)->GnYP[ii] = coefy * sub->Oy[ii] / setting->dy;
    (*data)->GnYM[ii] = coefy * sub->Oy[kk] / setting->dy;
    (*data)->GnCt[ii] = sub->Z[ii] + (*data)->GnXP[ii] + (*data)->GnYP[ii] + (*data)->GnXM[ii] + \
      (*data)->GnYM[ii];
    // avoid non-singularity of the matrix
    if ((*data)->GnCt[ii] == 0)
...
void updateVelocitySub(Data **data, Maps *map, Sub *sub, Config *setting)
{
  ...
  if (setting->useSubgrid == 1)
  {
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      if (sub->Vx[ii] > 0)
      {
        (*data)->uuXP[ii] = (*data)->EnXP[ii] - (setting->g * setting->dt * sub->Nx[ii] / \
          sub->Vx[ii]) * ((*data)->surf[map->iPjc[ii]] - (*data)->surf[ii]) ;/// (1 + (*data)->DragXP[ii]);
        //(*data)->uuXP[ii] = (*data)->EnXP[ii] - (setting->g * setting->dt / \
          setting->dx) * ((*data)->surf[map->iPjc[ii]] - (*data)->surf[ii]);
      }
      if (sub->Vy[ii] > 0)
      {
        (*data)->vvYP[ii] = (*data)->EnYP[ii] - (setting->g * setting->dt * sub->Oy[ii] / \
          sub->Vy[ii]) * ((*data)->surf[map->icjP[ii]] - (*data)->surf[ii]) ;/// (1 + (*data)->DragYP[ii]);
        //(*data)->vvYP[ii] = (*data)->EnYP[ii] - (setting->g * setting->dt / \
          setting->dy) * ((*data)->surf[map->icjP[ii]] - (*data)->surf[ii]);
      }
    }
  }
  else if (setting->useSubgrid == 2)
...
{
  int ii;
  double Cw = 0.7, CFLmax = 0.24/setting->subR, wtfA;
