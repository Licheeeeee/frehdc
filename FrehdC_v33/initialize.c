...
void Init(Data **data, Maps **map, IC **ic, BC **bc, Sub **sub, Bath *bath, Config *setting, int irank, int nrank);
...
void Init(Data **data, Maps **map, IC **ic, BC **bc, Sub **sub, Bath *bath, Config *setting, int irank, int nrank)
{
  ...
  // read restart file
  if (setting->isRestart == 1 & irank == 0)
  {printf("Setting up initial conditions ...\n");}
  if (setting->isRestart == 1)
  {readRestartFile(ic, &setting);}
...
void readIC(IC **ic, Bath *bath, Config *setting)
{
  ...
  (*ic)->restartU0 = malloc(setting->N2CI * sizeof(double));
  (*ic)->restartV0 = malloc(setting->N2CI * sizeof(double));
  (*ic)->restartSurf0 = malloc(setting->N2CI * sizeof(double));
  if (setting->useScalar == 1)
  {
    ...
    else
    {
      ...
      (*ic)->restartS0 = malloc(setting->N2CI * sizeof(double));
      char sicname[50];
...
void initFieldValues(Data **data, Bath *bath, IC *ic, Config *setting, int irank)
{
  ...
  else
  {
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      (*data)->uuXP[ii] = ic->restartU0[ii+irank*setting->N2ci];
      (*data)->vvYP[ii] = ic->restartV0[ii+irank*setting->N2ci];
      (*data)->surf[ii] = ic->restartSurf0[ii+irank*setting->N2ci] + bath->offset[0];
      if (setting->useScalar == 1)
      {(*data)->S[ii] = ic->restartS0[ii+irank*setting->N2ci];}
    }
  }
}
