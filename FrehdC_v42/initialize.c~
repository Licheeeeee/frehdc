...
void Init(Data **data, Maps **map, IC **ic, BC **bc, Sub **sub, Bath *bath, Config *setting, int irank, int nrank)
{
  ...
  if (irank == 0)
    {printf("Enforcing boundary conditions ...\n");}
  ...
  // initialize subgrid
  if (irank == 0)
    {printf("Initializing subgrid variables ...\n");}
...
//      mpiexchange((*sub)->Zym, *map, setting, irank, nrank);
    ...
    writeText("Combining subgrid variables ...\n", irank);
  ...
  int ii, col;
    ...
    for (ii = 0; ii < setting->N2ci; ii++)
    {
      ...
      if (setting->useScalar == 1)
      {
        ...
        {
	  (*data)->S[ii] = ic->S0;
	  // scalaric for UCBV8V5M/V6M
	  //printf("1 %f\n",setting->dx);
	  if (setting->dx == 1.0)
	    {
	      if (irank == 15) {(*data)->S[ii] = 20.0;}
	      else if (irank == 14 & ii >= 11520) {(*data)->S[ii] = 20.0;}
	    }
	  if (setting->dx == 16.0 & irank == 1 & ii >= 1440 ) 
	    {(*data)->S[ii] = 20.0;}
	}
        else
    ...
    if (setting->bcType == 3 && setting->useConstSurf0 == 1)
    {
        dsurf = (0.6-0.4) / setting->NY;
        for (ii = 0; ii < setting->N2ci; ii ++)
        {
            col = floor(ii / setting->NX);
            (*data)->surf[ii] = 0.4 + (irank*setting->ny + col)*dsurf + bath->offset[0];
        }
    }
}
